      ...
      <main className="max-w-[1400px] mx-auto px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_440px] gap-6">
          ...
          <section className="space-y-4">
            ...
            <Panel title="AUTO-PREEMPTION DAEMON" icon={Crown}>
              <div className="flex items-center justify-between mb-3">
                <div className="font-mono text-[10px] text-[#9ca3af] flex-1 pr-4">
                  Background loop that scans for blocked SGE jobs (or — without SGE — uses
                  priority rules with literal usernames) and{" "}
                  <span className="text-amber-400">automatically preempts</span> the
                  lowest-priority holder so the high-priority user can check out without
                  manual intervention.
                  <br />
                  <span className="text-[#6b7280]">Tip:</span> set{" "}
                  <span className="text-emerald-400">SGE Enabled</span> above for production
                  use — the loop reads{" "}
                  <span className="text-emerald-400">qstat -s p -u &apos;*&apos; -xml</span>{" "}
                  + per-job{" "}
                  <span className="text-emerald-400">hard resource_list:</span> to detect
                  what license each blocked job is waiting on.
                </div>
                <button
                  className="btn-brutal flex items-center gap-1.5 text-[10px] py-1"
                  onClick={async () => {
                    try {
                      ...
                      toast.success(
                        `Tick complete: scanned ${r.scanned} request(s), actioned ${r.actioned}`,
                      );
                    } catch (e) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="md:col-span-2">
                  <Toggle
                    label="Auto-preempt Loop Enabled"
              ...
              <div className="mt-3 flex items-start gap-2 text-[10px] font-mono text-[#9ca3af]">
                <Info size={12} className="mt-0.5 shrink-0" />
                <div>
                  Without SGE, the loop falls back to a heuristic: every priority rule whose{" "}
                  <code className="text-white">user_pattern</code> is a literal username
                  (no wildcards) is treated as &quot;this user wants this feature&quot;.
                  If that feature is saturated and the user has higher priority than a
                  current holder, the loop preempts. Wildcard patterns are ignored in
                  heuristic mode — they only work with real SGE signal.
                </div>
              </div>
            </Panel>
          </section>
